...
fun TodayScreen(
    ...
    LazyColumn(
        ...
        item {
            ...
            updateTag?.let { tag ->
                SectionCard(Modifier.fillMaxWidth().clickable(onClick = onOpenUpdate)) {
                    ...
                    Text("Tap to install the latest version.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
            }
            if (tampered) {
                SectionCard(Modifier.fillMaxWidth()) {
                    Text("Modified device detected", color = Color(0xFFFFB74D), fontWeight = FontWeight.SemiBold)
                    Text("Your data stays encrypted and tracking works normally.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
            }
        }
        ...
        item {
            OdometerHeader(
                heroBikeKm = if (showLifetime) (lifetime?.distanceBikeM ?: 0.0) else (today?.distanceBikeM ?: 0.0),
                allModesKm = if (showLifetime) (lifetime?.distanceAllM ?: 0.0) else (today?.distanceAllM ?: 0.0),
                showLifetime = showLifetime,
                onToggle = { showLifetime = !showLifetime },
            )
        }

        if (live.enabled) item { LiveCard(live) }
        ...
        item {
            Button(
                ...
                colors = ButtonDefaults.buttonColors(
                    containerColor = if (live.enabled) MaterialTheme.colorScheme.error else MaterialTheme.colorScheme.primary
                ),
            ) {
                Icon(if (live.enabled) Icons.Filled.Stop else Icons.Filled.PlayArrow, null)
                Text(if (live.enabled) "  Stop tracking" else "  Start tracking", fontWeight = FontWeight.SemiBold)
            }
        }
        ...
        item {
            SectionCard {
                NavRow(Icons.Filled.Speed, "Speedometer", "Full-screen dial for your handlebar mount", onClick = onOpenSpeedo)
            }
        }
        ...
        if (trips.isEmpty()) {
            item {
                Text(
                    "No rides yet today. Your first ride starts the odometer.",
            ...
            item {
                SectionCard(Modifier.fillMaxWidth()) {
                    trips.forEachIndexed { i, leg ->
                        if (i > 0) Divider()
                        CompactTripRow(leg) { onOpenTrip(leg.id) }
                    }
                }
            }
        }
...
@Composable
private fun OdometerHeader(heroBikeKm: Double, allModesKm: Double, showLifetime: Boolean, onToggle: () -> Unit) {
    SectionCard(Modifier.fillMaxWidth().clickable(onClick = onToggle)) {
        Text(
        ...
        Row(verticalAlignment = Alignment.Bottom) {
            Text(animatedKm(heroBikeKm), fontSize = 56.sp, fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onSurface)
            Text("  km", style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.onSurfaceVariant, modifier = Modifier.padding(bottom = 12.dp))
        }
        Text(
            "All modes ${Format.km(allModesKm)} km · tap to see ${if (showLifetime) "today" else "lifetime"}",
...
private fun LiveCard(live: com.soumik.stark.tracking.service.LiveState) {
    SectionCard(Modifier.fillMaxWidth()) {
        val label = when (live.state) {
            ...
            com.soumik.stark.tracking.service.TrackState.ARMED -> "WAITING FOR A RIDE"
            else -> ""
        }
...
@Composable
private fun Divider() {
    Box(Modifier.fillMaxWidth().height(1.dp).padding(vertical = 0.dp).background(MaterialTheme.colorScheme.onSurface.copy(alpha = 0.06f)))
}

@Composable
private fun CompactTripRow(leg: Leg, onClick: () -> Unit) {
    Row(
